 ...
 * under the License.
 *
 * @copyright  Copyright (c) 2013 PivotPass (http://www.dot.bm)
 * @license    http://www.apache.org/licenses/LICENSE-2.0  Apache 2.0
 * @version    $Id:$
 */

if (!function_exists('curl_init')) {
    throw new Exception('PivotPass needs the CURL PHP extension.');
}
if (!function_exists('json_decode')) {
    throw new Exception('PivotPass needs the JSON PHP extension.');
}
if (!function_exists('mime_content_type') && !(function_exists('finfo_file') && function_exists('finfo_open') && defined('FILEINFO_MIME_TYPE'))) {
    throw new Exception('PivotPass needs the FILEINFO PHP extension.');
}

/**
 * This class is the core of PivotPass.
 *
 * <p>You can start PivotPass using your app key. You can then create passes from templates, change values of passes or add passes to passbook.</p>
 * <code>
...
class PivotPass
{
    ...
    /**
     * PivotPass API Endpoint
     * @var string PivotPass API Endpoint
     */
    private $_endpoint = 'https://api.PivotPass.com/v1';
    ...
    /**
     * @var \PivotPass Shared class instance of PivotPass when using PivotPass::start
     */
    private static $_instance = null;

    /**
     * Allowed image types that are supported by PivotPass and Passbook
     */
     ...
     * Default user agent
     */
    const USER_AGENT = 'PivotPassSDK-PHP/0.6.1';

    /**
     * Creates a new PivotPass instance
     *
    ...
    /**
     * Start PivotPass with an app key
     *
     * @param string $appKey App Key
     * @param null $endpoint API Endpoint
     * @param bool $debug
     * @return PivotPass Shared PivotPass instance.
     */
     ...
     *
     * @return object Pass
     * @throws PivotPassApiException API Exception
     */
     ...
     * @param array $values Values for the placeholders
     * @param array $images Images that should be used for the pass
     * @see PivotPass::createPassFromTemplate()
     * @return object Pass
     * @throws PivotPassApiException API Exception
     */
     ...
     *
     * @param object $pass The pass created with PivotPass::createPassFromTemplate() or PivotPass::createPassFromTemplateWithName()
     ...
     * @throws PivotPassApiException
     * @throws PivotPassApiUnauthorizedException
     * @throws PivotPassApiValidationException
    ...
    private function _restCall($httpMethod, $resource, $content = null, $multipart = false)
    {
        ...
        if ($error_code) {
            ...
            throw new PivotPassApiException("Please check your PHP / libcurl version and ensure that the latest security protocols are supported ($error_message)", $error_code);
        }
        ...
        if ($httpCode == 422) {
            // Validation Error
            throw new PivotPassApiValidationException($response);
        }

        if ($httpCode == 401) {
            throw new PivotPassApiUnauthorizedException();
        }

        if ($httpCode < 200 || $httpCode >= 300) {
            throw new PivotPassApiException($response, $httpCode);
        }
...
/**
 * Class PivotPassWebhookReceiver
 */
class PivotPassWebhookReceiver {
 ...
 */
class PivotPassApiException extends Exception
{
 ...
 */
class PivotPassApiUnauthorizedException extends PivotPassApiException
{
 ...
 */
class PivotPassApiValidationException extends PivotPassApiException
{
